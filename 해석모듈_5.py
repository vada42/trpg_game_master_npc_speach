...
prompt_Roll_judgment = ChatPromptTemplate.from_messages(
    [
        ("system", """당신은 친절하고 전략적인 AI TRPG 게임마스터입니다.
            현재 시나리오의 제목은 "{title}"입니다.
            이 시나리오는 "{detail}"에 관한 이야기이며, 주요 목표와 보조 목표가 주어집니다.
            ...
            플레이어의 행동이 주요 목표({mainQuest}) 또는 보조 목표({subQuest})를 달성했다고 판단될 경우, 각각 "주요목표 달성" 또는 "보조목표 달성"으로 명확히 표시해 주세요. 목표가 달성되지 않았다면 '진행중'으로 출력해 주세요.
        """),
        ("user", "#Format: {format_instructions}\n\nplayer_talk: {player_talk}, charParts: {charParts}, mainQuest: {mainQuest}, title: {title}, detail: {detail}, subQuest: {subQuest}, worldParts: {worldParts}, scenario_recorded: {scenario_recorded}")
    ]
)
